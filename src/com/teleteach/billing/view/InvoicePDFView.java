...
//  pdfFilename = args[0].trim();
 ...
 public void createPDF (int billNo){
  ...
  try {
   String path = "C:/Users/Sanat Mishra/Desktop/" + billNo;
   docWriter = PdfWriter.getInstance(doc , new FileOutputStream(path));
   doc.addAuthor("Kirti saurabh");
   doc.addCreationDate();
   doc.addProducer();
   doc.addCreator("Tele-Teach Pvt Ltd.in");
   ...
   for(int i=0; i < 100; i++ ){
    if(beginPage){
     ...
     String billNos = Integer.toString(billNo);
     generateHeader(doc, cb , billNos);
     y = 615; 
    }
 ...
 private void generateLayout(Document doc, PdfContentByte cb)  {
  ...
  try {
   ...
   createHeadings(cb,22,633,"Item NO.");
   ...
   //add the images
   Image companyLogo = Image.getInstance("bin/images/Customer.png");
 ...
 private void generateHeader(Document doc, PdfContentByte cb, String billNos)  {
	 
  try {
 
   createHeadings(cb,200,750,"Company Name   : Tele-Teach Pvt Ltd");
   ...
   createHeadings(cb,482,723,billNos);
  
   Date date = Calendar.getInstance().getTime();
   DateFormat dateFormat = new SimpleDateFormat("yyyy-mm-dd hh:mm:ss");
   String strDate = dateFormat.format(date);
   createHeadings(cb,482,703,strDate);
